...
export default function FejlecIkonok() {
  ...
  function Kosar({
    ...
    return (
      <div
        className="w-max flex flex-col absolute top-12 right-0 bg-white rounded-md px-4 pt-3 pb-5 shadow-[0_0_10px_0_rgba(0,0,0,0.5)]"
  ...
  return (
    <div className="md:flex w-1/3 justify-end hidden relative">
      ...
      {cartOpen &&
        (cartItems.length > 0 ? (
          <Kosar setCartOpen={setCartOpen} />
        ) : (
          <div
            className="absolute top-12 right-0 bg-white rounded-md px-4 py-3 shadow-[0_0_10px_0_rgba(0,0,0,0.5)]"
            onMouseLeave={() => setCartOpen(false)}
          >
            A kosár üres
          </div>
